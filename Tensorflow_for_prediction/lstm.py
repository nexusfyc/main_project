...
#模型训练 fit network
history = model.fit(train_X, train_y, epochs=500, batch_size=128,validation_data=(test_X,test_y) , verbose=2,
                        shuffle=False)

# #输出 plot history
pyplot.plot(history.history['loss'], label='lstm_train')
pyplot.plot(history.history['val_loss'], label='lstm_test')
pyplot.legend()
pyplot.show()
...
#画出真实数据和预测数据
pyplot.plot(inv_yhat,label='lstm_prediction')
pyplot.plot(inv_y,label='lstm_true')
pyplot.legend()
pyplot.show()
